...
def vendor_shipment(store, delivery_time, reorder_list, shipment_items):
    ...
    # The shipments must be inserted into the database
        ...
        orders_rem = "SELECT COUNT(store_id) FROM order_request WHERE store_id = %s AND order_status = 0"
        params5 = (store,)
        ...
        all_remaining_orders = "SELECT COUNT(request_id) FROM order_request order_status = 0"
        cursor.execute(all_remaining_orders)
        print(all_remaining_orders[0])
